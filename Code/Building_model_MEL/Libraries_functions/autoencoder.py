...
def build_VAE_Skipp_menoscomplejo(X_train, latent_space_dim, epsilonstd, alpha, learning):
...
def build_VAE_Skipp_partial(input, latent_space_dim, epsilonstd, alpha):
    ...
    x3= layers.Conv2D(128, 3, activation="relu", strides=2, padding="same")(x)
    x = layers.Conv2D(256, 3, activation="relu", strides=2, padding="same")(x3)
    x5= layers.Conv2D(512, 3, activation="relu", strides=2, padding="same")(x)
...
def build_VAE_skipp(input, latent_space_dim, epsilonstd, alpha, learning):
    # Compute VAE loss
    def VAE_loss(x_origin,x_out):
        error = x_origin - x_out
        reconstruction_loss = K.mean(K.square(error), axis=[1, 2, 3])
 ...
 #           path,
  #          monitor="loss",
   #         verbose=1,
    ...
    learning_rate=learning), loss=VAE_loss)
        return model
    def load_model_weights(cls, save_folder=".", name=None):
        autoencoder=cls
